...
The Luna Helm chart simplifies deployment of the {kafka-short} extension.

The following steps explain how to deploy a {luna-streaming} Helm chart to create a simple {pulsar-short} cluster with the {product} extension ready to use.
...
The following command creates a Helm release named `my-pulsar-cluster` using the xref:installation:attachment$values.yaml[{company} Luna Helm chart] within a Kubernetes namespace named `datastax-pulsar`.
...
+
--
To use the {kafka-short} CLI or a {kafka-short} client with {product}, you use your {luna-streaming} {pulsar-short} tenant as the {kafka-short} bootstrap server.
