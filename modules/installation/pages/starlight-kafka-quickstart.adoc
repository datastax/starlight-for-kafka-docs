...
You use your {pulsar-short} tenant's connection details with a {kafka-short} client to produce and consume messages with {product}.
...
[#establish-the-kafka-protocol-handler-for-astra-streaming]
== Use {product} with {astra-stream}

Enable the {product} protocol handler in your {astra-stream} {pulsar-short} cluster, get the connection details for your {kafka-short} client, and then use a {kafka-short} client to interact with your {pulsar-short} cluster.

=== Enable {product} in {astra-stream}
...
. Use your tenant's connection details to produce and consume messages with the {kafka-short} CLI or a {kafka-short} client script, as demonstrated in the following examples.

=== Connect to {astra-stream} with the {kafka-short} CLI

This example uses command line tools included with the https://kafka.apache.org/downloads[{kafka} tarball]:

. Enable {product} and get the `ssl.properties` connection details, as explained in <<establish-the-kafka-protocol-handler-for-astra-streaming>>.
...
=== Connect to {astra-stream} with a {kafka-short} Java client
...
[#establish-the-kafka-protocol-handler-for-ibm-elite-support-for-apache-pulsar]
...
=== Deploy {luna-streaming} with {product} enabled
...
Then you'll use a {kafka-short} client to interact with your cluster.
...
====
After deploying the Helm chart and enabling port forwarding, your applications can communicate with {pulsar-short} as if it were a real {kafka-short} host, as demonstrated in the following {kafka-short} CLI and {kafka-short} client examples.

=== Connect to {luna-streaming} with the {kafka-short} CLI or a {kafka-short} client

[TIP]
====
You can modify the <<establish-the-kafka-protocol-handler-for-astra-streaming,{astra-stream} examples>> for your {luna-streaming} cluster by replacing the connection details with those for your cluster.
====
...
The following example uses command line tools included with the https://kafka.apache.org/downloads[{kafka} tarball], and it connects on `localhost:9092` because port forwarding was enabled in <<establish-the-kafka-protocol-handler-for-ibm-elite-support-for-apache-pulsar>>:
...
Install and configure the {product} protocol handler in your {pulsar-short} cluster, and then use the {kafka-short} CLI or a {kafka-short} client to interact with your {pulsar-short} cluster.

=== Deploy and configure {product} on a self-managed {pulsar-short} cluster
...
. Use your cluster connection details with the {kafka-short} CLI or a {kafka-short} client, as demonstrated in the following examples.

=== Connect to a self-managed {pulsar-short} cluster with the {kafka-short} CLI or a {kafka-short} client

[TIP]
====
You can modify the <<establish-the-kafka-protocol-handler-for-astra-streaming,{astra-stream} examples>> for your self-managed {pulsar-short} cluster by replacing the connection details with those for your cluster.
====
...
The following example uses command line tools included with the https://kafka.apache.org/downloads[{kafka} tarball], and it assumes port forwarding is enabled to connect on `localhost:9092`:
...
Build the {product} protocol handler from source if you want to customize the code or contribute to the project:
...
* xref:configuration:starlight-kafka-configuration.adoc[configure and manage {product}]
* xref:operations:starlight-kafka-proxy.adoc[Install {product} as a proxy extension]