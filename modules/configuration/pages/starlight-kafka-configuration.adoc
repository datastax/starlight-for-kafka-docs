...
{product} shares the same configuration files with the {pulsar-short} broker, e.g. `conf/broker.conf` or `conf/standalone.conf`. The log configuration is modified in `conf/log4j2.yaml` file:
