...
{product} shares the same configuration files with the {pulsar-short} broker, e.g. `conf/broker.conf` or `conf/standalone.conf`. The log configuration is modified in `conf/log4j2.yaml` file as below:
[source,yaml]
----
Logger:
...
{pulsar-short} is a multi-tenant system that requires users to specify the http://pulsar.apache.org/docs/en/concepts-multi-tenancy/[tenant and namespace]. While most {kafka-short} users just specify the short topic name, {product} provides the following configurations to specify the default namespace.
