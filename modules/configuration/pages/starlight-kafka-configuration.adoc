= Configure the {product} extension
:navtitle: Configuration

This page describes configuration options for {product}.
...
|kafkaListeners
|Required comma-separated list of URIs that {product} listens on and the listener names.
For example: `PLAINTEXT://localhost:9092,SSL://localhost:9093`

Each URI's scheme represents a listener name if `kafkaProtocolMap` is configured.
Otherwise, the scheme must be a valid protocol in `PLAINTEXT`, `SSL`, `SASL_PLAINTEXT`, or `SASL_SSL`.

If the hostname isn't set, it is bound to the default interface.

|kafkaAdvertisedListeners
|Optional listeners to publish to ZooKeeper for clients to use.
The format is the same as `kafkaListeners`.

|kafkaProtocolMap
|Optional comma-separated map of listener name and protocol.
For example: `PRIVATE:PLAINTEXT,PRIVATE_SSL:SSL,PUBLIC:PLAINTEXT,PUBLIC_SSL:SSL`

|listeners
|Deprecated.
Use `kafkaListeners` instead.
|===
...
{product} shares the same configuration files with the {pulsar-short} broker, which is `conf/broker.conf` or `conf/standalone.conf`.
The log configuration is modified in `conf/log4j2.yaml` file as below:
...
{pulsar-short} is a multi-tenant system that requires users to specify the http://pulsar.apache.org/docs/en/concepts-multi-tenancy/[tenant and namespace].
While most {kafka-short} users just specify the short topic name, {product} provides the following configurations to specify the default namespace.
...
|kafkaTenant
|The default tenant of {kafka-short} topics
|public

|kafkaNamespace
|The default namespace of {kafka-short} topics
|default

|kafkaMetadataTenant
|The tenant used for storing {kafka-short} metadata topics
|public
...
|The namespace used for storing {kafka-short} metadata topics
|kafka
...
|The allowed namespace to list topics with a comma separator, such as `public/default,public/kafka`.
...
This enhancement results in fully isolated tenants in the {pulsar-short} cluster, which isn't available in pure {kafka-short} where the system metadata is shared among all users.
...
|maxReadEntriesNum
a|The maximum number of entries that are read from the cursor once per time.
Increasing this value can make FETCH requests read more bytes each time.

[IMPORTANT]
====
{product} doesn't check the maximum byte limit.
Therefore, if this value is too large, then the response size can exceed the network limit.
====
|5
|===
...
|maxQueuedRequests
|Limit the queue size for request, like `queued.max.requests` in {kafka-short} server.
|500

|requestTimeoutMs
|Limit the timeout in milliseconds for request, like `request.timeout.ms` in {kafka-short} client.
If a request wasn't processed within the timeout limit, then {product} returns an error response to the client.
|30000

|connectionMaxIdleMs
|The idle connection timeout in milliseconds.
If the idle connection timeout (such as `connections.max.idle.ms` used in the {kafka-short} server) is reached, then the server handler closes this idle connection.
...
|Connection close delay on failed authentication: This is the time (in milliseconds) by which connection close is delayed on authentication failure, like `connection.failed.authentication.delay.ms` in {kafka-short} server.
|300
...
|{product} metrics exposed to prometheus rollover latency in seconds.
|60
|===
...
|kopSslProtocol
|{kafka-short} SSL configuration map with: SSL_PROTOCOL_CONFIG = ssl.protocol
|TLS

|kopSslProvider
|{kafka-short} SSL configuration map with: SSL_PROVIDER_CONFIG = ssl.provider
|

|kopSslCipherSuites
|{kafka-short} SSL configuration map with: SSL_CIPHER_SUITES_CONFIG = ssl.cipher.suites
|

|kopSslEnabledProtocols
|{kafka-short} SSL configuration map with: SSL_ENABLED_PROTOCOLS_CONFIG = ssl.enabled.protocols
|TLSv1.2, TLSv1.1, TLSv1

|kopSslKeystoreType
|{kafka-short} SSL configuration map with: SSL_KEYSTORE_TYPE_CONFIG = ssl.keystore.type
|JKS

|kopSslKeystoreLocation
|{kafka-short} SSL configuration map with: SSL_KEYSTORE_LOCATION_CONFIG = ssl.keystore.location
|

|kopSslKeystorePassword
|{kafka-short} SSL configuration map with: SSL_TRUSTSTORE_PASSWORD_CONFIG = ssl.truststore.password
|N/A
...
|{kafka-short} SSL configuration map with: SSL_TRUSTSTORE_LOCATION_CONFIG = ssl.truststore.location
|
...
|{kafka-short} SSL configuration map with: SSL_KEYMANAGER_ALGORITHM_CONFIG = ssl.keymanager.algorithm
|SunX509

|kopSslTrustmanagerAlgorithm
|{kafka-short} SSL configuration map with: SSL_TRUSTMANAGER_ALGORITHM_CONFIG = ssl.trustmanager.algorithm
|SunX509

|kopSslSecureRandomImplementation
|{kafka-short} SSL configuration map with: SSL_SECURE_RANDOM_IMPLEMENTATION_CONFIG = ssl.secure.random.implementation
|
|===
