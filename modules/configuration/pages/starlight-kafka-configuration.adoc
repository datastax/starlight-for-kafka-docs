...
|kafkaListeners
|Comma-separated list of URIs that Starlight for Kafka listens on and the listener names.
...
[NOTE]
====
Currently, Starlight for Kafka doesn't check the maximum byte limit. Therefore, if the value is too great, the response size may be over the network limit.
====
|5
|===
