...
The token can be created by Pulsar token tools.
...
S4K provides a built-in `AuthenticateCallbackHandler` that uses the authentication provider of Pulsar for authentication.
...
* **Existing TLS certificates (Recommended)**: Use the TLS certificates already configured for the Pulsar TLS endpoint to also connect the Kafka client. This is a new feature of Starlight for Kafka designed to make TLS connection fast and painless. Proceed to xref:starlight-kafka-security.adoc#pulsar-ssl[Create SSL connection with Pulsar].

* **Dedicated TLS certificates**: Create dedicated TLS certificates and modify the Pulsar broker and Kafka client. Instructions for this method are xref:starlight-kafka-security.adoc#manual-ssl[here].
...
. Configure TLS on the Pulsar broker.
...
----
+
These settings enable TLS on the broker, and then Starlight for Kafka picks up the TLS certificates automatically.
