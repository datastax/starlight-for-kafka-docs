...
- https://prometheus.io/docs/concepts/metric_types/#summary[Summary]: Similar to a histogram, a summary samples observations (usually things like request durations and response sizes).
Summaries also provide a total count of observations and a sum of all observed values.
Calculates configurable quantiles over a sliding time window.

== Alive channel metrics
...
|Summary
|The requests processing total latency for all {kafka-short} APIs.
...
|Summary
|The message publish latency to {pulsar-short} ManagedLedger.
...
|Counter
a|The producer message in stats.
...
|Counter
a|The producer message conversions in stats.
...
|Summary
|The prepare metadata latency in milliseconds before starting fetch from {pulsar-short} ManagedLedger.
...
|Counter
a|The consumer bytes out stats.
...
|Counter
a|The consumer message out stats.
...
|Counter
a|The consumer entries out stats.
...
|Counter
a|The consumer message conversions in stats.
