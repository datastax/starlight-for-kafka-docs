...
[NOTE]
====
The Proxy works only if the broker is working properly, because it simply forwards the requests to a broker. If the broker fails or it isn't accessible, then the Proxy answers with an error message.
====
