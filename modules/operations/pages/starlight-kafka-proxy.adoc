...
Starlight for Kafka allows you to deploy a https://github.com/apache/pulsar/wiki/PIP-99:-Pulsar-Proxy-Extensions[proxy extension] on the Pulsar proxy component. This allows the Kafka client to access your Pulsar cluster the same way as Pulsar clients do.
...
The Kafka proxy uses the same configuration as the Pulsar proxy.
...
== Discover Pulsar Brokers from the Proxy

The proxy uses the broker discovery service to discover the brokers.
The Pulsar broker doesn't advertise the address of the Kafka listeners, so the mapping between a broker and the actual TCP port that is listening for Kafka connections is done per convention.
...
This means that a broker on port 6650 for Pulsar protocol is mapped to Kafka port 19092, and port 6651 is mapped to 19093.
