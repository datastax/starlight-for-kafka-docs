...
In {product}, the replication factor is handled by BookKeeper and set at the namespace level.

{kafka-short} Streams sets its own internal `replication.factor` number, but {pulsar-short} applies its namespace replication factor from BookKeeper.
