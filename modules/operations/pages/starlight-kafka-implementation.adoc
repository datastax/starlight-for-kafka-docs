...
If a Kafka message does not have a `key`, both `ordering key` and `partition key` aren't set for the converted Pulsar message.
...
If a Kafka message does not have a `timestamp`, both `event time` and `publish time` aren't set for the converted Pulsar message.
