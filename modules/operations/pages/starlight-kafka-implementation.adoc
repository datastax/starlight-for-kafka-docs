...
dispatches every request to the Broker that owns the topic, so Kafka clients don't have knowledge of the internal topology of the cluster.
...
- If a Pulsar message doesn't have an `ordering key`, then the `partition key` is converted to a `key`.
...
- If a Pulsar message doesn't have an `ordering key` but it has a `partition key (yyy)`, then the message key of the converted Kafka message is `yyy`.
...
- If a Pulsar message doesn't have an `event time`, then the `publish time` is converted to `timestamp`.
...
- If a Pulsar message doesn't have an `event time` but it has a `publish time (1628826964821)`, then the message key of the converted Kafka message is `1628826964821`.
...
If a Kafka message doesn't have a `key`, then `ordering key` and `partition key` aren't set for the converted Pulsar message.
...
If a Kafka message doesn't have a `timestamp`, then `event time` and `publish time` aren't set for the converted Pulsar message.
